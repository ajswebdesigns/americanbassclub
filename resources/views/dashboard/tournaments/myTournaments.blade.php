                    ...
                    <div class="card-body">
                        ...
                        <h5 class="card-title @if($parti->payment_status != 'COMPLETED' && $parti->tournament->free_paid =='paid' && $parti->paymentMethod != 'cheque') text-danger @endif">
                            ...
                            @if($parti->payment_status != 'COMPLETED' && $parti->tournament->free_paid =='paid' && $parti->paymentMethod != 'cheque')
                                ...
                                <a href="{{route('processTransaction', $parti->id)}}" class="btn btn-primary float-right btn-sm">Make payment (${{$parti->tournament->tournament_price}})</a>
                            @endif

                            @if($parti->paymentMethod == 'cheque' && $parti->chequeStatus == 1)
                                <span class="btn btn-success float-right btn-sm">Check Status: Verified</span>
                            @elseif($parti->paymentMethod == 'cheque' && $parti->chequeStatus == 0)
                                <span class="btn btn-warning float-right btn-sm">Check Status: Not verified yet</span>
                            @endif
                        </h5>
                        <h6 class="card-subtitle mb-2 text-muted">
                            <i class="far fa-calendar-alt mr-1"></i> 
                            @if($parti->tournament->end_date == '0001-01-01')
                             T.B.D
                            @else
                                <span>{{ date('M d Y', strtotime($parti->tournament->date))}} - {{ date('M d Y', strtotime($parti->tournament->end_date))}}  </span> 
                            @endif
                            <br/>
